...
class kitti_object(object):
    ...
    def get_calibration(self, idx):
        assert idx < self.num_samples
        calib_filename = os.path.join(self.calib_dir, "%010d.txt" % (idx))
        return utils.Calibration(calib_filename)

    def get_label_objects(self, idx):
        assert idx < self.num_samples and self.split == "training"
        label_filename = os.path.join(self.label_dir, "%010d.txt" % (idx))
        return utils.read_label(label_filename)

    def get_pred_objects(self, idx):
        assert idx < self.num_samples
        pred_filename = os.path.join(self.pred_dir, "%010d.txt" % (idx))
    ...
    def get_depth(self, idx):
        assert idx < self.num_samples
        img_filename = os.path.join(self.depth_dir, "%010d.png" % (idx))
    ...
    def get_depth_pc(self, idx):
        assert idx < self.num_samples
        lidar_filename = os.path.join(self.depthpc_dir, "%010d.bin" % (idx))
    ...
    def isexist_depth(self, idx):
        assert idx < self.num_samples and self.split == "training"
        depth_filename = os.path.join(self.depth_dir, "%010d.txt" % (idx))
...
class kitti_object_video(object):
    ...
    def __init__(self, img_dir, lidar_dir, calib_dir):
        self.calib = utils.Calibration(calib_dir, from_video=False)
...
def viz_kitti_video(root_dir, args):
    video_path = root_dir
    # dataset = kitti_object_video(
    #     os.path.join(video_path, "/image_02"),
    #     os.path.join(video_path, "/velodyne"),
    #     os.path.join(video_path, "/calib"),
    # )
    # print(len())
    for i in range(400):
...
def show_lidar_with_depth(
    ...
    if depth is not None:
        ...
        if save:
            ...
            save_filename = os.path.join(vely_dir, "%010d.bin" % (data_idx))
            print(save_filename)
